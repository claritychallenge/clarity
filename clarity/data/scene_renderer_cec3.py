...
class SceneRenderer:
    ...
    def make_background_filename(self, background: dict, dataset) -> str:
        ...
        """
        data_type = background["type"]
        stem = self.paths.background.format(dataset=dataset, type=data_type)
        return f"{stem}/{background['name']}"


    def prepare_background_paths(self, scene):
        ...
        """
        logger.info("number of background: %s", len(scene["background"]))
        background_filename = self.make_background_filename(scene["background"][0], scene["dataset"])
        return background_filename

    def load_background_hoairs(self, scene):
        ...
        background_hoair = f"{hoair_path}/HOA_{room_id}_i1.wav"
        irs = wavfile.read(background_hoair)[1]
        return irs

    def load_background_signals(self, scene):
        ...
        """
        background_audio_paths = self.prepare_background_paths(scene)
        ...
        with warnings.catch_warnings():
            ...
            signal, _ = librosa.load(background_audio_paths, sr=None)

        #signal_starts = [backgrounds["offset"] for backgrounds in scene["backgrounds"]]
        signal_start = scene["background"][0]["offset"]

        signal_length = scene["background"][0]["time_end"] - scene["background"][0]["time_start"]
    ...
    def generate_hoa_signals(self, scene: dict) -> tuple:
        ...
        irs = self.load_background_hoairs(scene)
        background = self.load_background_signals(scene)

        hoa_background = [
            hoa.ambisonic_convolve(signal, ir, order=self.ambisonic_order)
            for (signal, ir) in zip(background, irs)
        ]
    ...
    def generate_binaural_signals(
        ...
        background = [
            hoa.binaural_mixdown(hoa_background, hrir, self.metadata.hrir_metadata)
            for hrir in hrirs
        ]
        ...
        sw_snr = better_ear_speechweighted_snr(
            ...
            background[start_time:end_time, :],
        )
        ...
        background *= sw_snr * hoa.dB_to_gain(-desired_snr)
        # Make mixture by summing target and interferers/backgrounds
        mix = [t + i for t, i in zip(targets, background)]
        norm = np.max(mix)
        ...
        for channel, (t, i, m, norm) in enumerate(
            zip(targets, background, mix, norms)
        ):
    ...
    def render_scenes(self, scenes: dict):
        ...
        for scene in tqdm(scenes):
            ...
            target, background, anechoic, head_turn = self.generate_hoa_signals(scene)
            ...
            self.generate_binaural_signals(
                scene, target, background, anechoic, output_path
            )
