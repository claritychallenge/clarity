...
class SceneRenderer:
    ...
    def make_backgrounds_filename(self, backgrounds: dict, dataset) -> str:
        ...
        """
        data_type = backgrounds["type"]
        stem = self.paths.backgrounds.format(dataset=dataset, type=data_type)
        return f"{stem}/{backgrounds['name']}"


    def prepare_backgrounds_paths(self, scene):
        ...
        """
        logger.info("number of backgrounds: %s", len(scene["backgrounds"]))
        backgrounds_filename = self.make_backgrounds_filename(scene["backgrounds"][0], scene["dataset"])
        return backgrounds_filename

    def load_backgrounds_hoairs(self, scene):
        ...
        backgrounds_hoair = f"{hoair_path}/HOA_{room_id}_i1.wav"
        irs = wavfile.read(backgrounds_hoair)[1]
        return irs

    def load_backgrounds_signals(self, scene):
        ...
        """
        backgrounds_audio_paths = self.prepare_backgrounds_paths(scene)
        ...
        with warnings.catch_warnings():
            ...
            signal, _ = librosa.load(backgrounds_audio_paths, sr=None)

        #signal_starts = [backgrounds["offset"] for backgrounds in scene["backgrounds"]]
        signal_start = scene["backgrounds"][0]["offset"]

        signal_length = scene["backgrounds"][0]["time_end"] - scene["backgrounds"][0]["time_start"]
        signal = signal[int(signal_start):int(signal_start + signal_length)]
        return [signal]
    ...
    def generate_hoa_signals(self, scene: dict) -> tuple:
        ...
        irs = self.load_backgrounds_hoairs(scene)
        backgrounds = self.load_backgrounds_signals(scene)

        hoa_backgrounds = [
            hoa.ambisonic_convolve(signal, ir, order=self.ambisonic_order)
            for (signal, ir) in zip(backgrounds, irs)
        ]

        # to make each interference signal has the same length in the scene
        #padded_interferers = [
        ...
        # to perform power equalization processing on multi-interferer signals
        #if self.equalise_loudness:
            #padded_backgrounds = hoa.equalise_rms_levels(hoa_backgrounds)
    ...
    def generate_binaural_signals(
        ...
        backgrounds = [
            hoa.binaural_mixdown(hoa_backgrounds, hrir, self.metadata.hrir_metadata)
            for hrir in hrirs
        ]
        ...
        sw_snr = better_ear_speechweighted_snr(
            ...
            backgrounds[start_time:end_time, :],
        )
        ...
        backgrounds *= sw_snr * hoa.dB_to_gain(-desired_snr)
        # Make mixture by summing target and interferers/backgrounds
        mix = [t + i for t, i in zip(targets, backgrounds)]
        norm = np.max(mix)
        ...
        for channel, (t, i, m, norm) in enumerate(
            zip(targets, backgrounds, mix, norms)
        ):
            for sig, sig_type in zip([t, i, m], ["target", "backgrounds", "mix"]):
    ...
    def render_scenes(self, scenes: dict):
        ...
        for scene in tqdm(scenes):
            ...
            target, backgrounds, anechoic, head_turn = self.generate_hoa_signals(scene)
            ...
            self.generate_binaural_signals(
                scene, target, backgrounds, anechoic, output_path
            )
