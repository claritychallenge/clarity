...
def test_audfilt():
    ...
    assert np.sum(np.abs(filter_params)) == pytest.approx(
        19.879915844855944, rel=REL_TOLERANCE, abs=ABS_TOLERANCE
    )
...
def test_make_smear_mat3_valid_input():
    ...
    assert np.sum(np.abs(f_smear)) == pytest.approx(
        2273.976168294156, rel=REL_TOLERANCE, abs=ABS_TOLERANCE
    )
...
def test_smear3():
    ...
    assert np.sum(np.abs(output_signal)) == pytest.approx(
        5066.986397433977, rel=REL_TOLERANCE, abs=ABS_TOLERANCE
    )
