...
def test_equalisation_cancellation() -> None:
    ...
    # check values
    assert np.sum(intermediate_grid) == pytest.approx(277.84700307853785)
    assert np.sum(p_ec_m) == pytest.approx(73.14293826967146)
...
def test_firstpartfunc() -> None:
    ...
    assert np.sum(result) == pytest.approx(405590.0256254587)
...
def test_secondpartfunc() -> None:
    ...
    assert result.shape == (n_taus, n_gammas)
    assert np.sum(result) == pytest.approx(127394.91897484378)
...
def test_thirdpartfunc() -> None:
    ...
    assert result.shape == (n_taus, n_gammas)
    assert np.sum(result) == pytest.approx(9483.482148264484)
...
def test_fourthpartfunc() -> None:
    ...
    assert result.shape == (n_taus, n_gammas)
    assert np.sum(result) == pytest.approx(75883.96658627654)
...
def test_stft() -> None:
    ...
    assert np.sum(stft_out) == pytest.approx(0.07977179832430759)
...
def test_remove_silent_frames() -> None:
    ...
    assert np.sum(xl_sil[0:300]) == pytest.approx(0.0)
    assert np.sum(xl_sil) == pytest.approx(128.15949932062742)
    assert np.sum(xr_sil) == pytest.approx(272.06229083012374)
    assert np.sum(yl_sil) == pytest.approx(413.65823132789836)
    assert np.sum(yr_sil) == pytest.approx(551.1857227061379)
...
def test_thirdoct() -> None:
    ...
    assert freq_low == pytest.approx([93.75, 109.375, 140.625])
    assert freq_high == pytest.approx([109.375, 140.625, 171.875])
    assert centre_frequencies == pytest.approx(
        np.array([[100.0, 125.99210499, 158.7401052]])
    )
    assert octave_band_matrix.shape == (3, 257)
    assert np.sum(octave_band_matrix) == pytest.approx(5.0)
