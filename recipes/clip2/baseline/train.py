...
from clip_dataset import build_train_dev_loaders, LyricIntelligibilityDataset, build_dataloader
from lyric_intelligibility_model import WhisperIntelligibilityModel, BetterEarLoss
...
def prepare_batch_whisper_stereo(
    ...
    feats_L = processor(left_list,  sampling_rate=16_000, return_tensors="pt").input_features.to(device)
    feats_R = processor(right_list, sampling_rate=16_000, return_tensors="pt").input_features.to(device)
...
def save_checkpoint(
    ...
    protected = {best_epoch} | {
        int(p.stem.replace("checkpoint_e", ""))
...
def train(cfg: DictConfig, train_loader, val_loader) -> None:
    ...
    for epoch in epoch_pbar:
        ...
        if epoch == cfg.train.finetune.epoch and not backbone_unfrozen:
            ...
            if isinstance(scheduler, ReduceLROnPlateau):
                scheduler = ReduceLROnPlateau(
                    ...
                    patience=cfg.train.get("lr_patience", 3), min_lr=1e-7,
                )
            else:
        ...
        val_m = run_epoch(
            model, val_loader, processor, device, better_ear,
            criterion, epoch, cfg.train.epochs,
        )
        ...
        lr_str = f"lr={lrs[0]:.2e}" if len(lrs) == 1 else f"lr_head={lrs[0]:.2e}  lr_bb={lrs[1]:.2e}"
        ...
        save_checkpoint(out_dir, epoch, model, optimizer, scheduler, val_m, cfg, is_best,
                        best_epoch=best_epoch, keep_last=keep_last)
...
def main(cfg: DictConfig) -> None:
    ...
    if better_ear:
        ...
        train_loader = build_dataloader(train_ds, batch_size=cfg.data.batch_size, shuffle=True,  num_workers=num_workers)
        val_loader   = build_dataloader(dev_ds,   batch_size=cfg.data.batch_size, shuffle=False, num_workers=num_workers)
    else:
