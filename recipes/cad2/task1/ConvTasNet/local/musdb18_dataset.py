...
class MUSDB18Dataset(torch.utils.data.Dataset):
    ...
    Notes:
        This is a modified versions of the MUSDB18 dataset from the version
        in Asteroid. It extend the Asteroid version to allow for targets
        ```vocals``` and ```background```. The background is the sum of all
        sources except vocals. This is useful for training models that
    ...
    def __getitem__(self, index):
        ...
        for source in self.sources:
            ...
            if self.mix_background:
                if source == "vocals":
                    audio_sources["vocals"] = audio
                else:
                    if "background" not in audio_sources:
                        audio_sources["background"] = audio
                    else:
                        audio_sources["background"] += audio
            else:
