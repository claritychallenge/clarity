...
class S2SBeamSearcher(S2SBaseSearcher):
    ...
    def _check_full_beams(self, hyps, beam_size):
        ...
        hyps : List
            ...
            Each inside list contains a list storing all the hypothesis for
                this sentence.
        beam_size : int
    ...
    def forward(self, src, tokens_bos, wav_lens, pad_idx):  # noqa: C901
        ...
        for j in range(self.n_ensembles):
            enc_states = self.model[j](src[j], tokens_bos, wav_lens, pad_idx)[
                0
            ].detach()
            batch_size = enc_states.shape[0]
            device = enc_states.device
            enc_lens = torch.round(enc_states.shape[1] * wav_lens).int()
            memory = self.reset_mem(batch_size * self.beam_size, device=device)
            all_enc_states.append(enc_states)
            all_enc_lens.append(enc_lens)
        ...
        if self.ctc_weight > 0:
            ...
            for j in range(self.n_ensembles):
                ctc_outputs = self.ctc_forward_step(all_enc_states[j], j)
                ...
                ctc_memory = None
                all_ctc_outputs.append(ctc_outputs)
                all_ctc_scorer.append(ctc_scorer)
                all_ctc_memory.append(ctc_memory)

        # Inflate the enc_states and enc_len by beam_size times
        all_inflate_enc_states = []
        all_inflate_enc_lens = []
        for j in range(self.n_ensembles):
            enc_states = inflate_tensor(all_enc_states[j], times=self.beam_size, dim=0)
            enc_lens = inflate_tensor(all_enc_lens[j], times=self.beam_size, dim=0)
            all_inflate_enc_states.append(enc_states)
            all_inflate_enc_lens.append(enc_lens)
        ...
        for t in range(max_decode_steps):
            ...
            all_log_probs_this_step = []
            all_attn = []
            for j in range(self.n_ensembles):
                log_probs_ens_n, memory_temp, attn = self.forward_step(
                    inp_tokens,
                    all_memory[j],
                    all_inflate_enc_states[j],
                    all_inflate_enc_lens[j],
                    j,
                )
                all_log_probs_this_step.append(log_probs_ens_n)
                all_memory[j] = memory_temp
                all_attn.append(attn)

            log_probs = torch.stack(all_log_probs_this_step, dim=0)
            ...
            if self.lm_weight > 0:
                ...
                lm_log_probs = lm_log_probs.unsqueeze(0).repeat([self.n_ensemble, 1, 1])
            ...
            if self.ctc_weight > 0:
                ...
                all_ctc_log_probs_this_step = []
                for j in range(self.n_ensembles):
                    ctc_log_probs_ens_n, ctc_memory_temp = all_ctc_scorer[
                        j
                    ].forward_step(g, all_ctc_memory[j], ctc_candidates, all_attn[j])
                    all_ctc_memory[j] = ctc_memory_temp
                    all_ctc_log_probs_this_step.append(ctc_log_probs_ens_n)

                ctc_log_probs = torch.stack(all_ctc_log_probs_this_step, dim=0)
                log_probs = log_probs + self.ctc_weight * ctc_log_probs

            log_probs = torch.log(
                torch.mean(torch.exp(log_probs), dim=0, keepdim=False)
            )
            ...
            # Index of which beam the current top-K output came from in (t-1) timesteps.
            predecessors = (
            ...
            for j in range(self.n_ensembles):
                all_memory[j] = self.permute_mem(all_memory[j], index=predecessors)
                if self.lm_weight > 0:
                    lm_memory = self.permute_lm_mem(lm_memory, index=predecessors)

                if self.ctc_weight > 0:
                    all_ctc_memory[j] = ctc_scorer.permute_mem(
                        all_ctc_memory[j], candidates
                    )

            # If using_max_attn_shift, the previous attn peak has to be permuted too.
            if self.using_max_attn_shift:
        ...
        if self.return_log_probs:
            ...
            for j in range(batch_size):
                ...
                best_logprobs = log_probs[j * self.topk]
                confidence = torch.exp(torch.sum(best_logprobs) / topk_lengths[j][0])
...
class S2STransformerBeamSearch(S2SBeamSearcher):
    ...
    **kwargs
    ...
    def __init__(self, modules, temperature=1.0, temperature_lm=1.0, **kwargs):
        ...
        self.model, self.fc, self.ctc_fc = [], [], []
        for j in range(self.n_ensembles):
            self.model.append(modules[j * 3])
            self.fc.append(modules[j * 3 + 1])
            self.ctc_fc.append(modules[j * 3 + 2])
